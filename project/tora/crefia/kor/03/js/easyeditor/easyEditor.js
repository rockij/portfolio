...
var easyConfig = {
	...
	width : "870px",			//기본 width
	height : "220px",			//기본 height
	//----------------------------------------------------
	filepath : "/static/js/easyeditor/",	
	imgpath : "/static/images/easyeditor",	//버튼 이미지경로
	...
	BtnList : {
		...
		center	: ["중앙정렬","center.gif"],
		justify : ["혼합정렬","justify.gif"], clean : ["스타일 가운데 맞춤", "clean.gif"], right : ["오른쪽정렬", "right.gif"],
...
function easyEditor(id)
{
	...
	//textarea에 설정한 사이즈로 다시 세팅한다. 2011.02.15 bbok
...
{	
	...
	this._divbtn.style.borderBottom = this.cfg.border;0
	...
	if(easyUtil.isIE)
	{
		easyUtil.addEvent(this._doc, "keydown", function(e) {
			...
			{	
				...
				range.pasteHTML("<br />"); 	// 공백라인이 두번 들어가는것을 막는다. 13.01.10 bbok
...
easyEditor.prototype.cmd = function(btn, order, value) 
{
	...
	{		
		...
		case "source":
			...
			{
				self._text.value = self.getHtml().trim();
			...
			{
				doc.body.innerHTML = self.getHtml().trim();
...
//easyEditor.getText
easyEditor.prototype.getText = function() 
{
    var s="";
    var hasInnerText = (document.getElementsByTagName("body")[0].innerText != undefined)? true : false;
    if(!hasInnerText){
    	s=this._doc.body.textContent;
    }else{
    	s=this._doc.body.innerText;
    }
    //if(easyUtil.isIE) s=this._doc.body.innerText;
    //else s=this._doc.body.textContent;
    return s;
}
...
//easyEditor.setHtml
easyEditor.prototype.setHtml = function(html) 
{	
	this._textarea.value = html;	
}
...
function (self){ 
	window.open("/static/js/easyeditor/table.htm","table","width=400,height=245,status=1");
} ];
...
function (self){
	...
	{	
		...
		{	
			var s="";
			s +="<input type='file' style='width:230px;font:8pt 돋움;color:gray' id='"+txt_id+"'/><br/>";
	...
	window.open("/admin/common/editorUploadView.do","table","width=400,height=100,status=1");
}]; 